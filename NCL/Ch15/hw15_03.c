// Purpose: Continue hw15_02, add char weekday[7] for struct and input by user
// File Name: hw15_03
...
{  
  ...
  do {
	...
	flag = Chk(check, flag);
	rewind(stdin);
	puts("Please input weekday(Ex: Monday):");
    check = scanf("%s", CurrentDay.weekday);
	flag = Chk(check, flag);
	rewind(stdin);
  ...
  printf("Date: %d/%d/%d %s", CurrentDay.year, CurrentDay.month, CurrentDay.day, CurrentDay.weekday);
